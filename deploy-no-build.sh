...
# Deploy script without TypeScript build (for pre-built projects)
# Usage: ./deploy-no-build.sh [server_ip] [server_user]

SERVER_IP=${1:-"your_server_ip"}
SERVER_USER=${2:-"root"}
PROJECT_NAME="mvz-bot"

echo "🚀 Deploying MVZ Telegram Bot to $SERVER_USER@$SERVER_IP (no build mode)"

# Check if dist directory exists
if [ ! -d "dist" ]; then
  echo "❌ dist directory not found. Please build project first with: npm run build"
  exit 1
fi

# Create tarball including dist directory
