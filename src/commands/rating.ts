...
export const finishGameCommand = async (ctx: BotContext): Promise<void> => {
  try {
    ...
    const formatInfo = gameSession.format === 'TRI' ? 'TRI (3×8)' : 'DUO (2×8)';
    const confirmMessage = `⚠️ Вы уверены, что хотите завершить игру ${formatInfo} и сбросить сессию?\n\n` +
      `Это действие:\n` +
      ...
      (gameSession.format === 'TRI' ? `🏆 TRI мини-матчи и рейтинги сохранены\n` : '') +
    ...
    const gameTypeLabel = gameSession.format === 'TRI' ? 'TRI игра' : 'игра';
    const statsSummary = gameSession.format === 'TRI' ? 
      `📊 Статистика, результаты матчей и TRI мини-матчи сохранены` :
      `📊 Статистика и результаты матчей сохранены`;
    
    await ctx.reply(`✅ ${gameTypeLabel.charAt(0).toUpperCase() + gameTypeLabel.slice(1)} завершена и сессия сброшена!\n\n` +
      `${statsSummary}\n` +
