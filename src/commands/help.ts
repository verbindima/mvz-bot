    ...
    if (isAdmin) {
      if (isPrivate) {
        ...
        if (CONFIG.TRI_MODE_ENABLED) {
          message += `<b>🎯 TRI режим (3×8 команды):</b>\n`;
          message += `/tri_init - Инициализировать TRI игру\n`;
          message += `/tri_confirm - Подтвердить TRI команды\n`;
          message += `/tri_cancel - Отменить TRI сессию\n`;
          message += `/tri_bulk_add - Пакетное добавление игроков\n`;
          message += `/tri_edit - Редактирование составов команд\n`;
          message += `/tri_results - Ввод результатов mini-матчей\n`;
          message += `/tri_mvp - Назначить MVP (до 3 игроков)\n`;
          message += `/tri_status - Статус всех TRI сессий\n\n`;
        }
